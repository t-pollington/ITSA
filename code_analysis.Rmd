---
title: "Code analysis: Impact of intensified control strategies on incidence of visceral leishmaniasis in a highly endemic district of Bihar, India"
...
# Preamble
Code run in *R* v.4.1.1 with *RStudio* v1.4.1717 and main package \texttt{surveillance} v1.19.1 and \texttt{hhh4addon} v0.0.0.0.9014. Unfortunately the general public will not be able to knit this document as the data is not publicly available. However we hope it will help those wishing to apply interrupted time-series analyses to their own data.
...
fig.loc = paste0(folder.loc,"/paper/manuscript_files/manuscript_svg")
setwd(folder.loc)
...
df = df[,c(1:33,100:166)] # drop death and pkdl cases
df[,101:133] = 0
...
# Study area
There were 33 study districts in Bihar State, as no consistent data for other 5.
...
#crude comparison
a <- sum(ka.sts@observed[25:36,33]) #jan2014-dec 2014 664, quoted in intro and results
b <- sum(ka.sts@observed[37:48,33]) #jan2015-dec 2015 411, quoted in intro and results
c <- sum(ka.sts@observed[49:60,33]) #jan2016-dec 2016 179, quoted in intro and results
d <- sum(ka.sts@observed[61:72,33]) #jan2017-dec 2017 172

(b-a)/a*100 # -38·1%, quoted in intro and results
(c-b)/b*100 # -56·4%, quoted in intro and results
```

## Spatial distribution
```{r morans}
nb <- poly2nb(IND_adm2, queen=F) # only polygons sharing an edge (not vertex) are nbours
...
The above figures show spatial correlation with neighbouring districts both before and during the intervention; Global Moran's I = 0·36, p = 0·002 (10,000 simulations) & Global Moran's I = 0·40, p < 1e-03, respectively.

# Model development using surveilance::hhh4 and hhh4addon
## Picking a cutoff for 'high/low endemicity'
We assign districts to low and high endemic groups based on visually picking a cutoff from the distribution of the number of villages and their respective cumulative study case numbers. It seems the distribution is a mixture of two and we chose the lowest 18 districts as 'low-endemic'; each group had a different overdispersion parameter.
...
VHHL.factor = HL.factor
levels(VHHL.factor) = c("L","H","VH")
VHHL.factor[c(18,27,33)] = "VH"
```

## Estimate a reasonable diagnosis-to-diagnosis interval for VL
Using infection-to-onset time from Chapman et al.'s 6-state Markov model (Table A8 of Quantification...) 135 (109–167) days, gives the incubation period mean. This was from BG but assumed to be the same for India. From the same data but a different analysis (Chapman et al.'s PNAS paper) we took the overdispersion term. Together we generated the incubation period distribution as a Negative Binomial distribution.

The infectiousness period was obtained from Jervis et al from their 2012-2013 study of onset-to-diagnosis time. We used the latter results from late 2013 and assumed these characteristics remained constant since 2013 until 2017. The times were read off the curve using pixel measurements and a log-normal distribution fitted. 

```{r estimate DxtoDx distribution}
# pixel length measurements from Jervis et al. figure S1. Q3 2012
zero.graph=652
min.graph=652
LQ.graph=618
med.graph=594
...
while (counter < N.desired) {
  ...
  # table S4 https://www.medrxiv.org/content/10.1101/2020.02.24.20023325v1.full.pdf and
  # p4 out of 37.
...
end.HL = matrix(data = 0, nrow = 72, ncol = 33)
end.HL[,which(HL.factor=="H")] = 1

end.VHHL.H = matrix(data = 0, nrow = 72, ncol = 33)
end.VHHL.H[,which(VHHL.factor=="H")] = 1
end.VHHL.VH = matrix(data = 0, nrow = 72, ncol = 33)
end.VHHL.VH[,which(VHHL.factor=="VH")] = 1

int.jan15constlag = calcDxconst(37)

# base model without end.HL & intervention
x <- list(
  ar = list(f = ~ 1 + vaishali),
  end = list(f = ~ 1, offset = population(ka.sts)),
...
summary(M1.base, idx2Exp = TRUE) # AIC: 10882.79

# base model with end.HL
x <- list(
  ar = list(f = ~ 1 + vaishali + int.jan15constlag),
  end = list(f = ~ end.HL, offset = population(ka.sts)),
  ne = list(f = ~ 1, weights = neighbourhood(ka.sts) == 1),
  family = HL.factor,
  funct_lag = Dx_lag,
  par_lag = 1,
  ...
  subset = 13:nrow(ka.sts))
M1.base = hhh4_lag(stsObj = ka.sts, control = x)
summary(M1.base, idx2Exp = TRUE) # AIC: 10886.67
...
x <- list(
  ...
  max_lag = 12,
  subset = 13:nrow(ka.sts))
M1.ENDseas = hhh4_lag(stsObj = ka.sts, control = x)
summary(M1.ENDseas, idx2Exp = TRUE) # AIC: 10721.52

# seasonality in END + AR
x <- list(
  ...
  max_lag = 12,
  subset = 13:nrow(ka.sts))
final.modelARseas <- hhh4_lag(stsObj = ka.sts, control = x)
summary(final.modelARseas, idx2Exp = TRUE) # AIC: 10517.49. 
# 10517.49-10886.67 = -369 (rounded)

# quick check with triple overdisp
x <- list(
  ar = list(f = addSeason2formula(~ 1 + vaishali + int.jan15constlag, period = ka.sts@freq)),
  end = list(f = addSeason2formula(~ 1 + end.HL, period = ka.sts@freq), offset = population(ka.sts)),
  ne = list(f = ~ 1, weights = neighbourhood(ka.sts) == 1),
  ...
  subset = 13:nrow(ka.sts))
final.modelARseas <- hhh4_lag(stsObj = ka.sts, control = x)
summary(final.modelARseas, idx2Exp = TRUE) # AIC: 10516.28 reduced a little bit
# need to investigate predictive performance

# plus end.VHHL
x <- list(
  ar = list(f = addSeason2formula(~ 1 + vaishali + int.jan15constlag, 
                                  period = ka.sts@freq)),
  end = list(f = addSeason2formula(~ 1 + end.VHHL.H + end.VHHL.VH, 
                                   period = ka.sts@freq), offset = population(ka.sts)),
  ne = list(f = ~ 1, weights = neighbourhood(ka.sts) == 1),
  family = VHHL.factor,
  funct_lag = Dx_lag,
  par_lag = 1,
  ...
  subset = 13:nrow(ka.sts))
final.modelARseas <- hhh4_lag(stsObj = ka.sts, control = x)
summary(final.modelARseas, idx2Exp = TRUE) # 10541.94, much higher with tri-endemic
...
x <- list(
  ...
  max_lag = 12,
  subset = 13:nrow(ka.sts))
final.modelARseasnopilot <- hhh4_lag(stsObj = ka.sts, control = x)
summary(final.modelARseasnopilot, idx2Exp = TRUE) # AIC: 10515.53.
```
...
names(Y) = X
Y # minima at month=45 however very little difference across the range. 
# Given that I will choose month = 37.
...
summary(final.modelintjan15, idx2Exp = TRUE, amplitudeShift = TRUE) #AIC 10517.49
changeoftrend.coeffs <- summary(final.modelintjan15, idx2Exp = TRUE) # work in exp() space

# final model (can we recode it to get the combined vars we need?)
end.L = ifelse(end.HL==0, 1, 0)
end.H = ifelse(end.HL==1, 1, 0)
x <- list(
  ar = list(f = addSeason2formula(~ 1 + vaishali + intervention, period = ka.sts@freq)),
  end = list(f = addSeason2formula(~ end.L + end.H - 1, period = ka.sts@freq), offset = population(ka.sts)),
  ne = list(f = ~ 1, weights = neighbourhood(ka.sts) == 1),
  family = HL.factor,
  funct_lag = Dx_lag,
  par_lag = 1,
  ...
  max_lag = 12,
  subset = 13:nrow(ka.sts))
final.modelintjan15 <- hhh4_lag(stsObj = ka.sts, control = x) # convergence problem

# get jackknife estimates for particular parameters requiring cov(X^2,Y^2) which is unobtainable from the hhh4-fitted model object i.e. final.modelintjan15

# create one-less ka.sts
sum(ka.sts@observed>0) # 1966 ">0" district-time observations are available
N.index = which(ka.sts@observed>0)
elm = matrix(nrow = 12, ncol = 2)
coef.list = rep(list(elm),1966)
ptm <- proc.time()
for (i in 1:1966) {
  temp.ka.sts = ka.sts
  temp.ka.sts@observed[N.index[i]] = temp.ka.sts@observed[N.index[i]] - 1
  # also remove from populationFrac too. Thus our interpretation of the jackknife here isn't just remove a case but also correctly update other elements of the STS object for conservation of human numbers!
  temp.ka.sts@populationFrac[N.index[i]] = temp.ka.sts@populationFrac[N.index[i]] - 1
  x <- list(
  ...
  temp.hhh4 <- hhh4_lag(stsObj = temp.ka.sts, control = x)
  if (temp.hhh4$convergence) {
    coef.list[[i]] = summary(temp.hhh4)$fixef
  } else {
    coef.list[[i]] = NA
  }
}
proc.time() - ptm # ~ 634s
save(coef.list, file = "coef.list.RData")

# table of coeffs
coef.table = matrix(nrow = 8, ncol = 4, dimnames = list(c("other","V","pilot","end.1","end.HL.HL","other*V","other*V*pilot","end.1*end.HL.HL"),c("modelmean","modelsd","jackmean","jacksd")))
coef.vector = unlist(coef.list)
other.vector = coef.vector[seq.int(from = 1, to = 47184, by = 24)]
V.vector = coef.vector[seq.int(from = 2, to = 47184, by = 24)]
pilot.vector = coef.vector[seq.int(from = 3, to = 47184, by = 24)]
end.1.vector = coef.vector[seq.int(from = 7, to = 47184, by = 24)]
end.HL.HL.vector = coef.vector[seq.int(from = 8, to = 47184, by = 24)]
othertimesV = other.vector + V.vector
othertimesVtimespilot = other.vector + V.vector + pilot.vector
end.1timesend.HL.HL = end.1.vector + end.HL.HL.vector
all(!is.na(coef.vector)) # PASS, all model runs converged
coef.table[1,3] = mean(exp(other.vector))
coef.table[2,3] = mean(exp(V.vector))
coef.table[3,3] = mean(exp(pilot.vector))
coef.table[4,3] = mean(exp(end.1.vector))
coef.table[5,3] = mean(exp(end.HL.HL.vector))
coef.table[6,3] = mean(exp(othertimesV))
coef.table[7,3] = mean(exp(othertimesVtimespilot))
coef.table[8,3] = mean(exp(end.1timesend.HL.HL))
coef.table[1,4] = sd(exp(other.vector))
coef.table[2,4] = sd(exp(V.vector))
coef.table[3,4] = sd(exp(pilot.vector))
coef.table[4,4] = sd(exp(end.1.vector))
coef.table[5,4] = sd(exp(end.HL.HL.vector))
coef.table[6,4] = sqrt(mean(exp(2*other.vector + 2*V.vector)) - (mean(exp(other.vector+V.vector)))^2)
coef.table[7,4] = sqrt(mean(exp(2*other.vector + 2*V.vector + 2*pilot.vector)) - (mean(exp(other.vector + V.vector + pilot.vector)))^2)
coef.table[8,4] = sqrt(mean(exp(2*end.1.vector + 2*end.HL.HL.vector)) - (mean(exp(end.1.vector + end.HL.HL.vector)))^2)
coef.table[1:5,1] = changeoftrend.coeffs[[9]][c(1,2,3,7,8)]
coef.table[1:5,2] = changeoftrend.coeffs[[9]][c(13,14,15,19,20)]
coef.table # jackmean is close to modelmean, however jacksd underestimates variance from 1-3 orders of magnitude! Alas it is the only solution we have, so we warn that variances computed from the jacksd estimator are likely to be underestimates.
...
nbstrap = 1
while (nbstrap < 11) {
  epsilon.bstrap = sample(epsilon, replace = FALSE)
  ...
  temp.hhh4 <- hhh4_lag(stsObj = temp.ka.sts, control = x)
  if (temp.hhh4$convergence) {
    coef.list[[nbstrap]] = summary(temp.hhh4)$fixef
    nbstrap = nbstrap + 1
  } 
}
proc.time() - ptm
save(coef.list, file = "coef.list.bstrap.RData")

# pre-pilot Vaishali
prepilot.vaishali = changeoftrend.coeffs[[9]][1] + changeoftrend.coeffs[[9]][2]
round(exp(prepilot.vaishali),3) # Vaishali-specific pre-pilot AR term 0·75
# pre-pilot CIs
# Var(X+Y) = Var(X)+Var(Y)+2Cov(X,Y)
prepilot.vaishali.se = sqrt((changeoftrend.coeffs[[9]][13]^2) + (changeoftrend.coeffs[[9]][14]^2) + (2*final.modelintjan15$cov[1,2]))
round(exp(prepilot.vaishali.se),3) #1.055
prepilot.lower = prepilot.vaishali - 1.96*prepilot.vaishali.se
prepilot.upper = prepilot.vaishali + 1.96*prepilot.vaishali.se 
round(exp(prepilot.lower),3) #0.676
round(exp(prepilot.upper),3) #0.832
...
round(exp(other.lower),3) #0.821
round(exp(other.upper),3) #0.869

# during-pilot CIs
duringpilot = changeoftrend.coeffs[[9]][1]+changeoftrend.coeffs[[9]][2]+
              changeoftrend.coeffs[[9]][3]
round(exp(duringpilot),3) #0.737
round((1-exp(changeoftrend.coeffs[[9]][3])) * 100,1) # 1·7% reduction for Vaishali during the 
...
round((1-exp(pilot.lower))*100,1) # 18.4%
round((1-exp(pilot.upper))*100,1) # -18.2%

# intervention jan15 up to dec 17 inclusive
# Var(X1+X2+X3) = Var(X1)+Var(X2)+Var(X3)+2Cov(X1,X2)+2Cov(X1,X3)+2Cov(X2,X3)
duringpilot.se = sqrt((changeoftrend.coeffs[[9]][13]^2)+(changeoftrend.coeffs[[9]][14]^2)+(changeoftrend.coeffs[[9]][15]^2)+(2*final.modelintjan15$cov[1,2])+(2*final.modelintjan15$cov[1,3])+(2*final.modelintjan15$cov[2,3]))
duringpilot.lower = duringpilot-1.96*duringpilot.se
duringpilot.upper = duringpilot+1.96*duringpilot.se
round(exp(duringpilot.lower),3) #0.635
round(exp(duringpilot.upper),3) #0.855

(1-exp(changeoftrend.coeffs[[9]][3])^36)*100 #46.9%

mean(popn.time.matrix[1,])*exp(changeoftrend.coeffs[[9]][7]) # interpreting endemic intercept point estimate on jan12. 0.0002368449/mo in total over the 33 study districts in their lower seasonal months in low endemic districts

exp(changeoftrend.coeffs[[9]][7] + changeoftrend.coeffs[[9]][8])
sqrt((changeoftrend.coeffs[[9]][19]^2) + (changeoftrend.coeffs[[9]][20]^2) + (2*final.modelintjan15$cov[7,8])) #se

mean(popn.time.matrix[1,])*exp(changeoftrend.coeffs[[9]][7]+changeoftrend.coeffs[[9]][8]+changeoftrend.coeffs[[9]][9]) # 0.96/mo for a high-endemic distt during
# a peak seasonal month 

# seasonality change
seasonal.ar = (as.numeric(plot(final.modelintjan15, type = "season")[[2]]))
round(diff(range(seasonal.ar))/min(seasonal.ar)*100, 3)
plot(final.modelintjan15, type = "season")

# ne contribution
round(exp(changeoftrend.coeffs[[9]][6])*100, 3)
```
...
par(family = "serif")
plot(final.modelintjan15$fitted.values[,33],final.model.residual[,33], 
     xlab = 'Fitted values', xlim = c(0,120), ylim = c(-3.2, 3.2), yaxt = "n",
     ylab = 'Model residuals (standardised)', xaxs = "i", yaxs = "i")
axis(2, at = c(-3,-1,0,1,3), labels = c("-3","-1","0","1","3"), las = 2)
abline(h = 0, lwd = 2, lty = 2)
dev.off()
```
Heteroskedasticity present so high variance when trying to predict low cases. But can't change variables or introduce a new one. May indicate that a subdistrict model with few cases may be difficult to fit.

```{r model-sensitivity-to-int.start.month}
i <- 1
X <- matrix(NA, nrow = 12, ncol = 13)
row.names(X) <- c('exp(ar.1)', 'exp(ar.vaishali)', 'exp(ar.intervention)', 'exp(ar.sin(2 * pi * t/12))', 'exp(ar.cos(2 * pi * t/12))', 'exp(ne.1)', 'exp(end.1)', 'exp(end.end.HL)', 'exp(end.sin(2 * pi * t/12))', 'exp(end.cos(2 * pi * t/12))',  'overdisp.H', 'overdisp.L')
# month 33=sep14; 37=jan15; 45=sep15
for (month in 33:45) {
intervention <- calcDxconst(month)
x <- list(  
...
signif(X[,5],digits = 3) # print the jan15 value
signif(apply(X, 1, max),digits = 3) # find max of each parameter

#intervention term varies the most, the rest just by a few percent except end.1.
(1.11-0.974)/0.974*100 #*14%
```

To assess select the best predictive model to simulate the counterfactual and intervention model we assessed their predictive performance. The test data was Jan 2016-Dec 2016. 2015 couldn't be used due to the 12 lags required. The simulation period was 2016-7.
...
# reference(base model) vs pilot and counterfactual on predictive performance
set.seed(2030)
models2compare = c("M1.base", "final.modelARseas", 
                   "final.modelARseasnopilot")
...
t(sapply(VLscores, colMeans, dims = 2))  
predictivePower(final.modelARseas, M1.base, seed = 2030) # 
...
# 9.9999e-06, limited by predictive power function at 100K tests
# par_value = 1.489985
...
SCORES <- c("logs", "rps", "dss", "ses")
VLscores = lapply(VLpred, scores, which = SCORES, individual = TRUE)
t(sapply(VLscores, colMeans, dims = 2)) # final.modelintjan15 better than all in predictive performance

par(mfrow = c(1,4))
for (m in models2compare){
  surveillance::pit(VLpred[[m]], plot = list(ylim = c(0, 1.4), main = m)) 
}
# some overdispersion in the final model (but it's better than the first 3) but there's
...
summary(final.modelwithout, idx2Exp = TRUE, amplitudeShift = TRUE) #AIC 10515.96
...
1. Visual check the 'no-change-of-trend' model can predict 2014 well based on 2013 fit.
...
y.pred <- oneStepAhead_hhh4lag(result = noint.2013, tp = time.period, type = 'rolling') # breaks, convergence problems
setwd(fig.loc)
...
dev.off()
```
Looks like a reasonable fit and prediction. Now do steps 2-4:
...
y.sim.final.model.pred <- simulate(final.modelARseas, nsim = no.sims, seed = starting.seed, subset = 37:72, y.start = y.start) # note both simulations get the same starting seed
y.sim.diff <- y.sim.noint.2012Dec14[,33,] - y.sim.final.model.pred[,33,]
averted <- colSums(y.sim.diff)
# hist(averted, main = 'Distribution of simulated estimates for cases averted (median = 569 cases)', xlab = 'KA cases averted', ylab = 'Number of simulations')
averted.median <- round(median(averted)) # median is better to report. 98
summary(averted)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -838  -41.00   96.00   96.41  233.00 1070.00
averted.mean = round(mean(averted))
round(mean(averted) - sd(averted)/sqrt(no.sims)*1.96) #95
round(mean(averted) + sd(averted)/sqrt(no.sims)*1.96) #98
sum(averted < 0) # no of sims with a negative effect, 31712/100Ksims
proc.time() - ptm # 80 mins
...
par(family = "serif")
plot(median.averted.t[3,], type = "l", ylim = c(-74,237), xaxs = "i", yaxs = "i", 
     xaxt = "n",
...
abline(h=0)
legend(x = c(2.5, 17), y = c(150, 210), text.width = 5, legend=c('median', 'upper/lower quartiles'), col=c("#56B4E9", "#56B4E9"), lty=c(1,2), lwd=4,
       seg.len = 4)
...
median(avertedsum/countersum*100) # 10.3% 

sum(ka.sts@observed[61:72,33])/popn.time.matrix[72,33]*10000 # VL case numbers in 2017 for section 4.3 'Control policy recommendations' 0.4144765
...
summary(extra16/counter16*100-100) #16.89%
...
summary(extra17/counter17*100-100) #30.51%
```
