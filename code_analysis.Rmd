...
# table of coeffs
coef.table = matrix(nrow = 8, ncol = 4, dimnames = list(c("other","V","pilot","end.1","end.HL.11","other*V","other*V*pilot","end.1*end.HL.11"),c("modelmean","modelsd","bstrapmean","bstrapsd")))
coef.vector = NULL
for (i in 1:100) {
...
# decided not to compute CIs for lower-priority results on non-pilot matters eg endemic/seasonal/neighbourhood contribution to avoid a sea of CIs. 

# endemic params
changeoftrend.coeffs[[9]][7]*changeoftrend.coeffs[[9]][8] # 1.54e-06, high
coef.table[8,4] # 2.92e-07, high
changeoftrend.coeffs[[9]][7] # 3.72e-08, low
changeoftrend.coeffs[[9]][19] # 1.48e-08, low

mean(popn.time.matrix[1,]) * changeoftrend.coeffs[[9]][7] * exp(changeoftrend.coeffs[[9]][9]*1) # interpreting endemic intercept point estimate on jan 2012 for a low-incidence setting(<11 cases/mo) at its seasonal maximum (thus sin()=1, exp(A_{END}*1) = and exp(A_{END})). 0.1525025/mo on average for the 33 study districts in high season. As this is far less than 1/mo, no point calculating what it would be in low season.
...
# seasonality change on AR. Not as interested as seasonality on END component as the END component generally has a far smaller contribution as per SI Fig. S5
plot(final.modelintjan15, type = "season") # read-off min & max of 2nd (AR compon.) graph 
(1.31 - 0.76)*100 # 55 % May (max), November (min)
