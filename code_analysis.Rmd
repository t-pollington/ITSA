...
# quick test if end.HL is end.HL.t where it can change for districts
end.HL.t = matrix(data = 0, nrow = 72, ncol = 33)
end.HL.t[ka.sts@observed >= 1] = 1
hist(ka.sts@observed, breaks = 259)
x <- list(
  ar = list(f = addSeason2formula(~ 1 + vaishali + intervention, period = ka.sts@freq)),
  ...
  max_lag = 12,
  subset = 13:nrow(ka.sts))
final.modelintjan15a <- hhh4_lag(stsObj = ka.sts, control = x)
final.modelintjan15a$convergence
summary(final.modelintjan15a, idx2Exp = TRUE, amplitudeShift = TRUE) #AIC 10343.97 !
...
model11 <- summary(hhh4_lag(stsObj = ka.sts, control = x), idx2Exp = TRUE)$fixef

# compare point ests and sd
cbind(summary(final.modelintjan15, idx2Exp = TRUE)$fixef, model11, model25)

boxplot(exp(residuals(final.modelintjan15)), use.cols = TRUE)# quite a range in the residuals by district, so makes sense to resample for the bootstrap by these groups
...
coef.vector = NULL
for (i in 1:100) {
  load(file = paste0("C:/Users/mochu/ITSA/bstrapresults/results/coef.list.bstrap",i,".RData"))
  coef.vector = c(unlist(coef.list), coef.vector)
}
other.vector = coef.vector[seq.int(from = 1, to = 48000, by = 24)]
V.vector = coef.vector[seq.int(from = 2, to = 48000, by = 24)]
pilot.vector = coef.vector[seq.int(from = 3, to = 48000, by = 24)]
end.1.vector = coef.vector[seq.int(from = 7, to = 48000, by = 24)]
end.HL.HL.vector = coef.vector[seq.int(from = 8, to = 48000, by = 24)]
...
stopifnot(all(!is.na(coef.vector))) # PASS, all model runs converged

hist(exp(other.vector))
hist(exp(V.vector))
hist(exp(pilot.vector))
hist(exp(end.1.vector))
hist(exp(end.HL.HL.vector))
hist(exp(othertimesV))
hist(exp(othertimesVtimespilot))
hist(exp(end.1timesend.HL.HL))
...
coef.table
range(coef.table[1:5,4]/coef.table[1:5,2]*100)
