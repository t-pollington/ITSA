...
# Preamble
Code run in *R* v.4.0.0 with *RStudio* v1.2.5042 and main package \texttt{surveillance} v1.18.0 and \texttt{hhh4addon} v0.0.0.0.90. Unfortunately the general public will not be able to knit this document as the data is not publicly available. However we hope it will help those wishing to apply interrupted time-series analyses to their own data.
...
library(fanplot) # fanplot
library(reshape2) # melt()
library(stats) # ts()
library(spdep) # nbOrder(), moran's I analysis
library(surveillance) # poly2adjmat(), hhh4_spacetime classes
library(sp) # shapefiles
library(latex2exp) # latex in graphs
library(abind) # combining matrices into an array
library(hhh4addon) # distributed lag
folder.loc = "C:/Users/mochu/OneDrive - University of Warwick/RMRI_analysis_tim/intensive_vaishali/paper1analysis/vaishali_VL_interrupted_time_series"
...
```{r rank-changes-of-yoy-absolute-cases-or-yoy-monthly-changes}
...
for (i in 37:nrow(df3)) {
  vaishali.yoy.rank <- c(vaishali.yoy.rank,as.numeric(rank(df3[i,35:67])[32]))
}
round(mean(vaishali.yoy.rank)) # 19th largest VL reduction averaged over 36 monthly y-o-y 
...
for (i in 13:36) {
  vaishali.yoy.pre.rank <- c(vaishali.yoy.pre.rank,as.numeric(rank(df3[i,68:100])[32]))
}
round(mean(vaishali.yoy.pre.rank)) # 26th largest VL reduction averaged over 24 monthly 
...
y <- vaishali.yoy.rank
wilcox.test(x, y, alternative = "t") #p<1e-07

vaishali.cases.rank <- NULL
for (i in 37:72) {
  vaishali.cases.rank <- c(vaishali.cases.rank,
                           as.numeric(rank(-1 * (rank(df3[i,1:33])))[32]))
}
round(mean(vaishali.cases.rank)) # 9th highest VL incidence averaged over 36 monthly 
# incidence (1st=largest VL incidence) during 2015-7
vaishali.cases.pre.rank <- NULL
for (i in 1:36) {
  vaishali.cases.pre.rank <- c(vaishali.cases.pre.rank,
                               as.numeric(rank(-1 * (rank(df3[i,1:33])))[32]))
}
round(mean(vaishali.cases.pre.rank)) # 3rd highest VL incidence averaged over 36 monthly 
# incidences (1st=largest VL incidence) during 2012-4
# Wilcoxon two-sample test with continuity correction
x <- vaishali.cases.pre.rank
y <- vaishali.cases.rank
wilcox.test(x, y, alternative = "t") # p<1e-09
```
Vaishali had the 19th largest (1st=largest negative change) VL reduction, averaged over 36 monthly ranks based on year-on-year percentages during 2015-7, versus before the intervention having only the 26th largest VL reduction averaged over 24 months during 2013-4. Wilcoxon two-sample one-tailed test with continuity correction, p=1e-07.

It had the 9th highest (1st=highest) VL incidence averaged over 36 monthly incidence ranks during 2015-7, versus before the intervention when it had the 3rd highest VL incidence averaged over 36 monthly incidences (1st=highest VL incidence) during 2012-4. Same Wilcoxon test, p<1e-09

Considering these two results it appears that during the intervention period Vaishali changed significantly. In the next section we will use a time-series model to account for factors other than the intervention effect that may have caused this to come about.
...
## Estimate a reasonable serial interval for VL
...
summary(M1.base, idx2Exp = T) # AIC: 10882.79
...
x <- list(
  ...
  subset = 13:nrow(ka.sts))
M1.base = hhh4_lag(stsObj = ka.sts, control = x)
summary(M1.base, idx2Exp = T) # AIC: 10886.67
...
summary(M1.ENDseas, idx2Exp = T) # AIC: 10721.52
...
summary(final.modelARseas, idx2Exp = T) # AIC: 10517.49. 
...
x <- list(
  ...
  subset = 13:nrow(ka.sts))
final.modelARseas <- hhh4_lag(stsObj = ka.sts, control = x)
summary(final.modelARseas, idx2Exp = T) # AIC: 10516.28 reduced a little bit
...
x <- list(
  ...
  subset = 13:nrow(ka.sts))
final.modelARseas <- hhh4_lag(stsObj = ka.sts, control = x)
summary(final.modelARseas, idx2Exp = T) # 10541.94, much higher with tri-endemic
...
summary(final.modelARseasnopilot, idx2Exp = T) # AIC: 10515.53.
```
...
X[i] <- month
Y[i] <- summary(intsensitiv, idx2Exp = T, amplitudeShift = T)$AIC
i <- i + 1
}
...
summary(final.modelintjan15, idx2Exp = T, amplitudeShift = T) #AIC 10517.49
changeoftrend.coeffs <- summary(final.modelintjan15) # work in model (log) link space

# pre-pilot Vaishali
prepilot.vaishali = changeoftrend.coeffs[[9]][1] + changeoftrend.coeffs[[9]][2]
round(exp(prepilot.vaishali),3) # Vaishali-specific AR term for before the intervention was 
# applied 0·75
# pre-pilot CIs
...
params <- summary(y, idx2Exp = T, amplitudeShift = T)
...
summary(final.modelwithout, idx2Exp = T, amplitudeShift = T) #AIC 10515.96
...
summary(noint.2013, amplitudeShift = T, idx2Exp = T)
...
summary(noint.Dec14, amplitudeShift = T, idx2Exp = T)
no.sims <- 100000 
...
abline(h=0)
legend(x = c(2.5, 16), y = c(150, 210), text.width = 5, legend=c('median', 'upper/lower quartiles'), col=c("#56B4E9", "#56B4E9"), lty=c(1,2), lwd=4)
