...
The infectiousness period was obtained from Jervis et al (doi: 10.1186/s13071-017-2530-9) from their 2012–2013 study of onset-to-diagnosis time. We used the latter results from late 2012 and assumed these characteristics remained constant since 2012 until 2017. The times were read off the curve using pixel measurements and a log-normal distribution fitted  (SI §S6). 
...
round(mean(Dx.distrib)) # mean 6mo Dx distrib. Match sliding windows to these (§3.3)
...
summary(M1.base, idx2Exp = TRUE) # AIC: 10593·25 (§3.4)
...
summary(final.model.nopilot, idx2Exp = TRUE) # AIC: 10286·16 (§3.4)
```
...
names(Y) = X
Y # minima at months = 36:38 so will choose midpoint as month = 37. (§3.4)
```
...
prepilot.vaishali # Vaishali-specific pre-pilot AR term 0·680 (Table 1)

# other districts CIs
changeoftrend.coeffs[[9]][1] # 0·709  (§3.4)
...
changeoftrend.coeffs[[9]][4] # 0·270 (Table 1)
changeoftrend.coeffs[[9]][16] # 0·022 (Table 1)
changeoftrend.coeffs[[9]][5] # -0·664 (Table 1)
changeoftrend.coeffs[[9]][17] # 0·047 (Table 1)
changeoftrend.coeffs[[9]][9] # 0·377 (Table 1)
changeoftrend.coeffs[[9]][21] # 0·133 (Table 1)
changeoftrend.coeffs[[9]][10] # 0·515 (Table 1)
changeoftrend.coeffs[[9]][22] # 0·171 (Table 1)

# endemic parameters
changeoftrend.coeffs[[9]][7]*changeoftrend.coeffs[[9]][8] # 1·54e-06, high (Table 1)
changeoftrend.coeffs[[9]][7] # 3·72e-08, low (Table 1)
changeoftrend.coeffs[[9]][19] # 1·48e-08, low (Table 1)

mean(popn.time.matrix[1,]) * changeoftrend.coeffs[[9]][7] * exp(changeoftrend.coeffs[[9]][9]*1) # interpreting endemic intercept point estimate on Jan 2012 for a low-incidence setting(<11 cases/mo) at its seasonal maximum (thus sin()=1, and so exp(A_{END}*1) = exp(A_{END})). 0·1525025/mo on average for the 33 study districts in high season. As this is far less than 1/mo, no point calculating what it would be in low season. (§3.4)
...
# November (min) 0·76x , May (max) 1·31x (§3.4)

# neighbourhood contribution
changeoftrend.coeffs[[9]][6] # 4·77e-03 (Table 1)
...
t(sapply(VLscores, colMeans, dims = 2)) # final & counterfactual models have lower RPS than base model (2·82). counterfactual (2·46) has higher predictive power than final model (2·50). (§3.4)
...
averted.median <- round(median(averted)) # median is better to report. 354. (§3.5)
summary(averted) # (§3.5)
...
summary((extra16/counter16*100))-100 # 36·8 LQ, 93·6 med, 202·1 UQ (§3.5)
...
summary((extra17/counter17*100))-100 # -44·20 LQ, 29·1 med, 137·9 UQ (§3.5)
```
