...
boxplot(residuals(final.modelintjan15), use.cols = TRUE) # quite a range in the residuals by district, so makes sense to resample by district grouping for this bootstrap. See coef.bstrap.R which performed this on the High Performance Cluster.

# table of coeffs
coef.table = matrix(nrow = 8, ncol = 4, dimnames = list(c("other","V","pilot","end.1","end.HL.HL","other*V","other*V*pilot","end.1*end.HL.HL"),c("modelmean","modelsd","bstrapmean","bstrapsd")))
coef.vector = NULL
for (i in 1:100) {
  load(file = paste0("C:/Users/mochu/ITSA/bstrapresults1/results1/coef.list.bstrap",i,".RData"))
...
hist(exp(end.HL.HL.vector)) # quite varied
hist(exp(othertimesV))
hist(exp(othertimesVtimespilot))
hist(exp(end.1timesend.HL.HL))

withinHDI <- function(x) {
  range = as.numeric(hdi(x, credMass = 0.95))
  whatswithin = ((x >= range[1]) & (x <= range[2]))
  x = x[whatswithin]
  return(x)
}

# from histogram analysis going to use 95% HD interval to trim distributions for a more robust estimate
other.vector = withinHDI(other.vector)
V.vector = withinHDI(V.vector)
pilot.vector = withinHDI(pilot.vector)
end.1.vector = withinHDI(end.1.vector)
end.HL.HL.vector = withinHDI(end.HL.HL.vector)
othertimesV = withinHDI(othertimesV)
othertimesVtimespilot = withinHDI(othertimesVtimespilot)
end.1timesend.HL.HL = withinHDI(end.1timesend.HL.HL)
...
coef.table # appears that bootstrap estimates for the first 5 rows (that can be compared with model output) show that bootstrapping tends to underestimate the sd by around ...
100-(round(range(coef.table[1:5,4]/coef.table[1:5,2]*100))) # 52--97%
...
sum(averted < 0) # no of sims with a negative effect, 1949/100Ksims
